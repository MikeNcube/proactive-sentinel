# Proactive Sentinel - Mission

## Project Identity
- Product: Proactive Sentinel
- Type: Multi-tenant security operations centre service
- Licence: Open Source

## Target Audience
- Organisations needing automated threat detection without a dedicated SOC team
- Multi-tenant SaaS platforms requiring per-tenant security monitoring

## Problems This Solves
- Eliminates manual log review for common threat patterns
- Automates alert correlation into MITRE ATT&CK attack chains
- Detects UX degradation signals alongside security threats
- Provides per-tenant isolation so one client never sees another's data

## Success Metrics
- Zero cross-tenant data leakage under any condition
- Zero false negatives on critical threat patterns in the test suite
- Alert correlation produces valid MITRE ATT&CK technique IDs on every chain
- SHA-256 deduplication eliminates all duplicate alerts within a session
- Auto-ban triggers correctly on velocity violations and reverses on manual unban

## Out of Scope
- LLM-based threat detection (until human-in-the-loop gate is designed and approved)
- Cross-tenant threat correlation (tenants are always isolated)
- Real-time streaming beyond Redis-based deduplication cache
