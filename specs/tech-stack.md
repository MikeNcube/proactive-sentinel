# Proactive Sentinel - Technical Stack

## Backend
- Language: Python 3.11
- Framework: Flask with application factory pattern
- Database: PostgreSQL 15
- Cache: Redis 7 for SHA-256 fingerprint deduplication
- ORM: SQLAlchemy 2.x with TypeDecorator for encrypted columns

## Security Stack
- JWT HS256 access and refresh tokens (separate secrets, separate expiry)
- bcrypt password hashing
- AES-256-GCM column-level encryption on phone, full name, address, MFA secret
- ENCRYPTION_KEY must always be separate from JWT secret
- Regex-based PII masking: South African IDs, Zimbabwe IDs, passport numbers
- Rate limiting via Flask-Limiter on all public endpoints
- Secure response headers: HSTS, X-Frame-Options, X-Content-Type-Options

## Detection Stack
- Rule-based detection for file-access spikes and UX degradation signals
- SHA-256 alert fingerprinting in Redis for deduplication
- MITRE ATT&CK technique ID classification for all attack chains
- Confidence scoring between 0.0 and 1.0 with documented calculation logic
- Per-IP and per-unit velocity detection with auto-ban and manual unban

## Observability
- OpenTelemetry: Flask and SQLAlchemy instrumentation
- OTLP/gRPC export
- Structured JSON logging with correlation IDs
- Correlation IDs must never include cross-tenant data

## Deployment
- Docker Compose: python:3.11-slim, PostgreSQL 15, Redis 7
- Non-root container user
- All environment variables injected at runtime, never hardcoded

## Decisions Already Made (agents must not override these)
- Redis keys must always be namespaced by tenant ID
- SHA-256 for fingerprinting, never MD5 or SHA-1
- MITRE ATT&CK as the only classification framework
- AES-256-GCM for column encryption, never AES-CBC
- Tenant isolation is non-negotiable at every layer
