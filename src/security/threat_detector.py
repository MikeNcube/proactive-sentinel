...
class ThreatDetector:
    ...
    def evaluate_request_velocity(self, ip: str, now: float | None = None) -> dict:
    ...
    def evaluate_ip_reputation(self, ip: str) -> dict:
        ip = self._normalize_ip(ip)
        if ip in self.known_bad_ips or ip in self.blocked_ips:
            return {"allowed": False, "reason": "blocked_ip"}
        return {"allowed": True}

    def register_violation(self, ip: str, reason: str) -> dict:
        ip = self._normalize_ip(ip)
